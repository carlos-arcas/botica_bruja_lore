...
- **Validación frontend básica (si aplica):** `npm run lint`, `npm run test:checkout-demo`, `npm run test:cuenta-demo`, `npm run test:api-base-url-env` y `npm run build`.
...
```
  ...
  - con ello, CI valida en un único paso auditable: readiness backend, `manage.py check`, tests críticos backend, integridad operativa/documental del repo y lint/tests de checkout+cuenta+contrato de entorno API/build frontend.
