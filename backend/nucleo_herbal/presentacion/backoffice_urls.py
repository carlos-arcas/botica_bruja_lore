"""Rutas API del backoffice seguro para Next.js."""

from django.urls import path

from .backoffice_views import (
    cambiar_publicacion_editorial_backoffice,
    cambiar_publicacion_producto_backoffice,
    cambiar_publicacion_ritual_backoffice,
    cambiar_publicacion_seccion_backoffice,
    actualizar_seleccion_fila_importacion_backoffice,
    adjuntar_imagen_fila_importacion_backoffice,
    confirmar_lote_importacion_backoffice,
    confirmar_validas_lote_importacion_backoffice,
    crear_lote_importacion_backoffice,
    descartar_fila_importacion_backoffice,
    detalle_lote_importacion_backoffice,
    cancelar_lote_importacion_backoffice,
    descartar_filas_importacion_backoffice,
    eliminar_imagen_fila_importacion_backoffice,
    revalidar_lote_importacion_backoffice,
    subir_imagen_backoffice,
    estado_backoffice,
    exportar_backoffice,
    guardar_editorial_backoffice,
    ajustar_inventario_backoffice,
    guardar_producto_backoffice,
    guardar_ritual_backoffice,
    guardar_seccion_backoffice,
    listado_editorial_backoffice,
    detalle_inventario_backoffice,
    listado_inventario_backoffice,
    listado_pedidos_backoffice,
    listado_productos_backoffice,
    listado_plantas_asociables_backoffice,
    listado_rituales_backoffice,
    marcar_pedido_enviado_backoffice,
    marcar_pedido_entregado_backoffice,
    marcar_pedido_preparando_backoffice,
    listado_secciones_backoffice,
)

urlpatterns = [
    path("estado/", estado_backoffice, name="backoffice-estado"),
    path("productos/", listado_productos_backoffice, name="backoffice-productos"),
    path("pedidos/", listado_pedidos_backoffice, name="backoffice-pedidos"),
    path("pedidos/<str:pedido_id>/preparando/", marcar_pedido_preparando_backoffice, name="backoffice-pedido-preparando"),
    path("pedidos/<str:pedido_id>/enviado/", marcar_pedido_enviado_backoffice, name="backoffice-pedido-enviado"),
    path("pedidos/<str:pedido_id>/entregado/", marcar_pedido_entregado_backoffice, name="backoffice-pedido-entregado"),
    path("inventario/", listado_inventario_backoffice, name="backoffice-inventario"),
    path("inventario/<str:producto_id>/", detalle_inventario_backoffice, name="backoffice-inventario-detalle"),
    path("inventario/<str:producto_id>/ajustar/", ajustar_inventario_backoffice, name="backoffice-inventario-ajustar"),
    path("productos/plantas-asociables/", listado_plantas_asociables_backoffice, name="backoffice-productos-plantas-asociables"),
    path("productos/guardar/", guardar_producto_backoffice, name="backoffice-producto-guardar"),
    path("productos/<str:producto_id>/publicacion/", cambiar_publicacion_producto_backoffice, name="backoffice-producto-publicacion"),
    path("rituales/", listado_rituales_backoffice, name="backoffice-rituales"),
    path("rituales/guardar/", guardar_ritual_backoffice, name="backoffice-ritual-guardar"),
    path("rituales/<str:ritual_id>/publicacion/", cambiar_publicacion_ritual_backoffice, name="backoffice-ritual-publicacion"),
    path("editorial/", listado_editorial_backoffice, name="backoffice-editorial"),
    path("editorial/guardar/", guardar_editorial_backoffice, name="backoffice-editorial-guardar"),
    path("editorial/<int:articulo_id>/publicacion/", cambiar_publicacion_editorial_backoffice, name="backoffice-editorial-publicacion"),
    path("secciones/", listado_secciones_backoffice, name="backoffice-secciones"),
    path("secciones/guardar/", guardar_seccion_backoffice, name="backoffice-seccion-guardar"),
    path("secciones/<int:seccion_id>/publicacion/", cambiar_publicacion_seccion_backoffice, name="backoffice-seccion-publicacion"),
    path("imagenes/subir/", subir_imagen_backoffice, name="backoffice-imagen-subir"),
    path("<str:modulo>/exportar/", exportar_backoffice, name="backoffice-exportar"),
    path("importacion/lotes/", crear_lote_importacion_backoffice, name="backoffice-importacion-crear-lote"),
    path("importacion/lotes/<int:lote_id>/", detalle_lote_importacion_backoffice, name="backoffice-importacion-detalle-lote"),
    path("importacion/lotes/<int:lote_id>/confirmar/", confirmar_lote_importacion_backoffice, name="backoffice-importacion-confirmar-lote"),
    path("importacion/lotes/<int:lote_id>/confirmar-validas/", confirmar_validas_lote_importacion_backoffice, name="backoffice-importacion-confirmar-validas"),
    path("importacion/lotes/<int:lote_id>/cancelar/", cancelar_lote_importacion_backoffice, name="backoffice-importacion-cancelar-lote"),
    path("importacion/lotes/<int:lote_id>/descartar/", descartar_filas_importacion_backoffice, name="backoffice-importacion-descartar-filas"),
    path("importacion/lotes/<int:lote_id>/revalidar/", revalidar_lote_importacion_backoffice, name="backoffice-importacion-revalidar-lote"),
    path("importacion/lotes/<int:lote_id>/filas/<int:fila_id>/seleccion/", actualizar_seleccion_fila_importacion_backoffice, name="backoffice-importacion-seleccion-fila"),
    path("importacion/lotes/<int:lote_id>/filas/<int:fila_id>/descartar/", descartar_fila_importacion_backoffice, name="backoffice-importacion-descartar-fila"),
    path("importacion/lotes/<int:lote_id>/filas/<int:fila_id>/imagen/", adjuntar_imagen_fila_importacion_backoffice, name="backoffice-importacion-adjuntar-imagen-fila"),
    path("importacion/lotes/<int:lote_id>/filas/<int:fila_id>/imagen/eliminar/", eliminar_imagen_fila_importacion_backoffice, name="backoffice-importacion-eliminar-imagen-fila"),
]
