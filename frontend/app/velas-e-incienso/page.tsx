...
export default async function PaginaVelasEIncienso({
  searchParams,
}: {
  searchParams?: Promise<Record<string, string | string[] | undefined>>;
}): Promise<JSX.Element> {
  const filtros = resolverFiltrosBoticaDesdeSearchParams((await searchParams) ?? {});
  const resultado = await obtenerProductosPublicosPorSeccion(VELAS_E_INCIENSO_PUBLICA.slug, filtros);

  return (
    <ContenedorPaginaComercial>
      <HeroSeccionPrincipal idSeccion="velas-e-incienso" nivelTitulo="h1" />
      <section aria-label={VELAS_E_INCIENSO_PUBLICA.ariaCatalogo} className="botica-natural__layout-catalogo">
        <aside className="botica-natural__rail-filtros" aria-label={VELAS_E_INCIENSO_PUBLICA.ariaFiltros}>
          <h2>Filtrar catalogo</h2>
          <PanelFiltrosBoticaNatural
            filtrosActivos={filtros}
            rutaSeccion={`/${VELAS_E_INCIENSO_PUBLICA.slug}`}
            textoAyuda="Ajusta los filtros para encontrar el encendido ritual que mejor acompana tu espacio."
          />
        </aside>
