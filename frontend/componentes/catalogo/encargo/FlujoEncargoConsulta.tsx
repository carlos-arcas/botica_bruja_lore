...
import { convertirCestaALineasSeleccion, convertirCestaAItemsEncargo, crearCestaVacia } from "@/contenido/catalogo/cestaRitual";
import { construirResumenHumanoSeleccion, resolverResumenEconomicoSeleccion } from "@/contenido/catalogo/seleccionEncargo";
...
export function FlujoEncargoConsulta({ slugPreseleccionado, cestaPreseleccionada, origenPreseleccionado }: Props): JSX.Element {
  const router = useRouter();
  const [cestaLocal, setCestaLocal] = useState(() => crearCestaVacia());
  const contexto = useMemo(
    () => resolverContextoPreseleccionado(slugPreseleccionado ?? null, cestaPreseleccionada ?? null, origenPreseleccionado ?? null),
    [cestaPreseleccionada, origenPreseleccionado, slugPreseleccionado],
  );
  const lineasSeleccion = useMemo(
    () => contexto.modo === "seleccion" ? convertirCestaALineasSeleccion(contexto.itemsPreseleccionados.length > 0 ? { lineas: contexto.itemsPreseleccionados.map((item) => ({ ...item, actualizadoEn: new Date().toISOString() })) } : cestaLocal) : [],
    [contexto, cestaLocal],
  );
  const itemsSeleccionados = useMemo(
    () => contexto.itemsPreseleccionados.length > 0 ? contexto.itemsPreseleccionados : convertirCestaAItemsEncargo(cestaLocal),
    [contexto.itemsPreseleccionados, cestaLocal],
  );
  const resumenSeleccion = useMemo(() => construirResumenHumanoSeleccion(lineasSeleccion), [lineasSeleccion]);
  const resumenEconomico = useMemo(() => resolverResumenEconomicoSeleccion(lineasSeleccion), [lineasSeleccion]);
  const [datos, setDatos] = useState<DatosConsulta>(() => construirEstadoInicialConsulta(contexto, lineasSeleccion));
  ...
  const estadoIdentificacion = useMemo(() => resolverEstadoIdentificacionCheckoutDemo(cuentaDemoActiva, continuarComoInvitado), [continuarComoInvitado, cuentaDemoActiva]);
  const productoSeleccionado = useMemo(() => PRODUCTOS_CATALOGO.find((producto) => producto.slug === datos.productoSlug) ?? null, [datos.productoSlug]);
  const canalCheckout: CanalCheckoutDemo = estadoIdentificacion.canalActivo;

  useEffect(() => {
    if (origenPreseleccionado === "seleccion" && contexto.itemsPreseleccionados.length === 0) {
  ...
  useEffect(() => {
    ...
    setDatos((previo) => combinarDatosIniciales(borrador?.datos ?? inicial, previo, cuenta, contexto.modo, resumenSeleccion));
  }, [contexto, lineasSeleccion, resumenSeleccion]);

  const actualizarCampo = (campo: keyof DatosConsulta, valor: string | boolean): void => setDatos((previo) => ({ ...previo, [campo]: valor }));

  const enviarConsulta = async (event: FormEvent<HTMLFormElement>): Promise<void> => {
    event.preventDefault();
    const lineas = construirLineasPedidoDemo(itemsSeleccionados, datos.productoSlug, datos.cantidad);
    const nuevosErrores = { ...validarSolicitudConsulta(datos, contexto.modo), ...validarCheckoutDemo(canalCheckout, estadoIdentificacion.cuentaActiva, lineas) };
    limpiarMensajesPrevios(setMensajeCopia, setMensajeCanal, setMensajeEnvio, setPedidoCreado, setEstadoEnvio);
    ...
    setResumen(construirResumenConsulta(datos, productoSeleccionado, contexto.modo, lineasSeleccion));
  ...
  return (
    <section className="bloque-home" aria-labelledby="titulo-encargo">
      ...
      <p>{contexto.modo === "seleccion" ? "Has llegado desde tu selección. Revisamos las líneas elegidas y tú editas solo tu intención real, sin arrastrar parches ni textos técnicos." : "Cuéntanos qué pieza te interesa y en qué contexto deseas usarla. Al final tendrás un texto listo para copiar o enviar por un canal real, si está configurado."}</p>

      {contexto.modo === "seleccion" ? (
        <article className={estilos.resumenProducto} aria-live="polite">
          <h2>Selección para encargo</h2>
          <ul>
            {lineasSeleccion.map((linea) => <li key={linea.id_linea}>{linea.cantidad} · {linea.nombre} · {linea.tipo_linea === "catalogo" ? "Catálogo" : linea.tipo_linea === "fuera_catalogo" ? "Fuera de catálogo" : "Sugerencia editorial"}</li>)}
          </ul>
        ...
        <article className={estilos.resumenProducto} aria-live="polite">
          ...
          {productoSeleccionado ? <><p><strong>{productoSeleccionado.nombre}</strong> · {productoSeleccionado.precioVisible}</p><p>{productoSeleccionado.subtitulo}</p></> : <p>No hay producto preseleccionado. Elige una pieza desde el formulario para continuar.</p>}
        </article>
      )}

      <BloqueIdentificacionCheckoutDemo canalActivo={canalCheckout} cuentaDemo={cuentaDemoActiva} onContinuarComoInvitado={() => setContinuarComoInvitado(true)} onIrCuentaDemo={() => { guardarBorradorCheckoutDemo(datos, continuarComoInvitado); router.push(construirRutaCuentaDemoConRetornoSeguro(RUTA_CUENTA_DEMO_RETORNO)); }} onUsarCuentaDemo={() => activarCuentaDemo(cuentaDemoActiva, setContinuarComoInvitado, setDatos)} />
      ...
      <form className={estilos.formulario} onSubmit={enviarConsulta} noValidate>
        ...
        <CampoFormulario etiqueta={contexto.modo === "seleccion" ? "Cantidad o formato a concretar" : "Cantidad o formato deseado"} name="cantidad" value={datos.cantidad} onChange={(valor) => actualizarCampo("cantidad", valor)} />
        <label>
        ...
        <button className="boton boton--principal" type="submit" disabled={estadoEnvio === "enviando"}>{estadoEnvio === "enviando" ? "Enviando pedido demo..." : "Enviar pedido demo"}</button>
      </form>

      <div className="hero-portada__acciones">
        <Link href="/colecciones" className="boton boton--secundario">Volver al catálogo</Link>
        {contexto.modo === "producto" && productoSeleccionado?.slug && <Link href={`/colecciones/${productoSeleccionado.slug}`} className="boton boton--secundario">Regresar a la ficha</Link>}
        {contexto.modo === "seleccion" && <Link href="/cesta" className="boton boton--secundario">Regresar a mi selección</Link>}
      </div>

      {estadoEnvio === "error" && <p className={estilos.error}>{mensajeEnvio}</p>}
      <ResumenEnvioEncargoDemo estadoCanalDescripcion={estadoCanal.descripcion} estadoCanalDisponible={estadoCanal.disponible} canales={estadoCanal.canales} construirHrefCanal={obtenerEnlaceCanal} onIntentoCanal={(tipo) => { if (!obtenerEnlaceCanal(tipo)) setMensajeCanal("No se pudo abrir el canal porque faltan datos válidos de configuración o resumen."); }} onCopiarResumen={copiarResumen} ctaSecundaria={estadoCanal.ctaSecundaria} mensajeCanal={mensajeCanal} mensajeCopia={mensajeCopia} resumen={resumen} estadoEnvio={estadoEnvio} pedidoCreado={pedidoCreado} productoSlug={productoSeleccionado?.slug} />
    </section>
  );
}

type CampoFormularioProps = { etiqueta: string; name: string; value: string; onChange: (valor: string) => void; error?: string; errorId?: string; type?: "text" | "email" | "tel" };
...
function CampoFormulario({ etiqueta, name, value, onChange, error, errorId, type = "text" }: CampoFormularioProps): JSX.Element {
  ...
  return <><label>{etiqueta}<input name={name} type={type} value={value} onChange={(event) => onChange(event.target.value)} aria-invalid={Boolean(error)} aria-describedby={idError} /></label>{error && <p id={idError} className={estilos.error}>{error}</p>}</>;
}

function activarCuentaDemo(cuentaDemo: ReturnType<typeof leerSesionCuentaDemo>, setContinuarComoInvitado: (valor: boolean) => void, setDatos: (actualizador: (previo: DatosConsulta) => DatosConsulta) => void): void {
  setContinuarComoInvitado(false);
  if (cuentaDemo?.email) setDatos((previo) => ({ ...previo, email: previo.email || cuentaDemo.email }));
}

function combinarDatosIniciales(base: DatosConsulta, previo: DatosConsulta, cuentaDemo: ReturnType<typeof leerSesionCuentaDemo>, modo: "producto" | "seleccion", resumenSeleccion: string): DatosConsulta {
  return {
    ...base,
    nombre: base.nombre || cuentaDemo?.nombre_visible || previo.nombre,
    email: base.email || cuentaDemo?.email || previo.email,
    mensaje: modo === "seleccion" ? base.mensaje || resumenSeleccion : base.mensaje || previo.mensaje,
