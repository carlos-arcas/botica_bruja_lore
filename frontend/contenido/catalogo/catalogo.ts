...
function construirImagenProducto(producto: ProductoBase): string {
...
function resolverPaletaVisual(categoria: CategoriaCatalogo, intencion: IntencionCatalogo): {
  fondo: string;
  acento: string;
  etiquetaCategoria: string;
} {
  const fondos: Record<IntencionCatalogo, string> = {
    calma: "#7a8f7b",
  ...
  const acentos: Record<CategoriaCatalogo, string> = {
    "mezcla-herbal": "#d5c49b",
    "ritual-guiado": "#caa8d2",
    herramienta: "#7ea0ad",
    "pack-regalo": "#d89b6d",
  };
  const etiquetas: Record<CategoriaCatalogo, string> = {
...
function escaparTextoSvg(texto: string): string {
  return texto
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
